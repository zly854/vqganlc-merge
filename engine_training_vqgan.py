...
def train_one_epoch(
    ...
    #for data_iter_step, images in enumerate(data_loader):
        ...
        loss, rec_loss, qloss, p_loss, g_loss, cls_loss, tk_labels, xrec = model(x, clip_images, label_cls, cur_iter, step=0)
        #cls_loss = F.cross_entropy(pred, label_cls, reduction='mean')
        #loss = loss + args.rate_cls * cls_loss
        ...
        if args.use_cblinear != 0:
            ...
            loss_scaler_ae(loss, opt_ae, parameters=model.parameters(), update_grad=(data_iter_step + 1) % accum_iter == 0)
        ...
        if cur_iter > args.disc_start and args.rate_d != 0:
            #with  torch.cuda.amp.autocast():
            d_loss, _, _, _, _, _, _, _ = model(x, clip_images, label_cls, cur_iter, step=1)
            opt_disc.zero_grad()
        ...
        if cls_loss is not None:
            clsloss_value = cls_loss.item()
            metric_logger.update(cls_loss=clsloss_value)
            misc.all_reduce_mean(clsloss_value)
            clsloss_value_reduce = misc.all_reduce_mean(clsloss_value)
        ...
        if log_writer is not None and cur_iter % 1000 == 0:
            ...
            if cls_loss is not None:
                log_writer.add_scalar("Iter/Cls Loss", clsloss_value_reduce, epoch_1000x)
            if cur_iter > args.disc_start and args.rate_d != 0:
    ...
    if log_writer is not None:
        ...
        if cls_loss is not None:
            log_writer.add_scalar("Epoch/Cls Loss", clsloss_value_reduce, epoch)
        if cur_iter > args.disc_start and args.rate_d != 0:
