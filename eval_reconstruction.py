...
class ImageNetDataset(Dataset):
    def __init__(self, data_root, image_size, model_path=None, max_words=30, n_class=1000, partition="train", device="cpu"):
...
def get_args_parser():
    ...
    parser.add_argument("--n_vision_words", default=1024, type=int)
    ...
    parser.add_argument("--merge_ratio", type=int, default=3, help="merge ratio")
    parser.add_argument("--dataset", type=str, default="ffhq", help="")
    parser.add_argument("--rate_cls", type=float, default=0.1, help="")
...
def main(args):
    ...
    if args.dataset == "imagenet":
        ...
        dataset_val = ImageNetDataset(
            data_root=args.imagenet_path, image_size=args.image_size,
            # model_path=args.model_path,
            max_words=args.max_seq_len, n_class=args.n_class, partition="val", device=device
        )
    else:
        print("FFHQ Dataset")
    ...
    for data_iter_step, [images, clip_image, label_cls] in enumerate(
        ...
        with torch.no_grad():
            _, _, _, _, _, pred, tk_labels, xrec = model(x, clip_image.to(device),label_cls, data_iter_step, step=0, is_val=True)
        ...
        metric_logger.update(acc=correct_total/num_images)
    ...
    print("ACC:", correct_total.item()/num_images)
    ...
    with open(os.path.join(args.output_dir, "recons.csv"), 'a') as f:
        f.write("FID, LPIPS PSNR, SSIM, ACC, Effective_Tokens \n")
        f.write("%.4f, %.4f, %.4f, %.4f, %.4f, %d \n"%(fid_value, lpips_total.item()/num_images, psnr_total.item()/num_images, ssim_total.item()/num_images, correct_total.item()/num_images ,efficient_token))
